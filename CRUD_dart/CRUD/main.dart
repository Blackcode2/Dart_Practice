...
void main() async {
    while (true) {
        ...
        if (pick == '1') {
            ...
            for(int i = 0; i < 10; i++) {
                if(pick == twoDList[i][0]){
                  	...
                  	//twoDList[i].addAll([id, name, age]); Why not working?
                  	print(twoDList);
                  	...
                  	while(twoDList[inputCount].isNotEmpty) {
						if(inputCount == 0) {
							var file = await File(filename).writeAsString('${twoDList[inputCount][0]} ${twoDList[inputCount][1]} ${twoDList[inputCount][2]}\n');
        ...
        } else if(pick == '4'){	//rmove information
			int count = 0;
			String? pick;
			while(true) {
				stdout.write('Delete one person\'s information -> press 1.\nDelete all information -> press 2: ');
                pick = stdin.readLineSync();
                if(pick != null) break;
                print('Try again.');
			}
			if(pick == '1') {
				...
				for(int i = 0; i < count; i++){
            		if(twoDList[i].isNotEmpty) {
						if(inputCount == 0) {
							var file = await File(filename).writeAsString('${twoDList[i][0]} ${twoDList[i][1]} ${twoDList[i][2]}\n');	// First line in new file
							inputCount++;
						} else {
							var file = await File(filename).writeAsString('${twoDList[i][0]} ${twoDList[i][1]} ${twoDList[i][2]}\n', mode: FileMode.append);
